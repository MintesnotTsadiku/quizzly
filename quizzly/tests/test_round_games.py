...
class TestRoundGameCatalog(IntegrationTestCase):
	...
	def test_bluffline_has_anonymous_submission_and_vote_phases(self):
		...
		created=create_session("bluffline",{"pack":pack,"seconds":15,"rounds":1});session,pin=created["session"],created["game_pin"]
		players={n:join_session(pin,n) for n in ("Liya","Dawit","Hana")};start_session(session);frappe.db.commit()
		try:
			for n,text in zip(players,("A ceremonial basket","An old measuring stick","A travel cloak"),strict=True):submit_action(pin,players[n]["participant_token"],"submit",str(uuid.uuid4()),{"value":text})
			state=gpe.get_state(session);transition=__import__("quizzly.games",fromlist=["get_game_module"]).get_game_module("bluffline").advance_state(gpe.context_for(frappe.get_doc("GP Session",session)),state,{"command":"deadline"});gpe.apply_transition(frappe.get_doc("GP Session",session),state,transition)
			liya=get_player_state(pin,players["Liya"]["participant_token"])["view"]
			self.assertEqual(liya["phase"],"vote_open");self.assertNotIn("A ceremonial basket",[c["value"] for c in liya["choices"]])
			submit_action(pin,players["Liya"]["participant_token"],"vote",str(uuid.uuid4()),{"value":"truth"})
		finally:self.cleanup_session(session)
