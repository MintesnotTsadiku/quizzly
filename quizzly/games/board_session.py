...
from quizzly.games.puzzles.rules import KEYS as PUZZLE_KEYS

BOARD_GAME_KEYS = PUZZLE_KEYS | {"grid-conquest"}


def is_board(doc):
	return (
		doc.game_key in BOARD_GAME_KEYS
...
def command(doc, action, payload, participant=None):
	...
	with frappe.cache.lock(f"gp:board:{doc.name}", timeout=30):
		...
		if hasattr(module, "puzzle_command") and action in {
			"edge",
			"cell",
			"note",
			"undo",
			...
			transition = module.puzzle_command(
				ctx, state, action, payload, participant.name if participant else None
			)
		...
		if action in {"place_mark", "edge", "cell", "note", "undo", "place", "remove"} and participant:
			engine.record_action(
				doc,
				engine.get_round_name(doc.name, state["module_state"]["round_index"]),
				participant,
				action,
				{**payload, "revision": state["version"]},
