...
async function api(page, method, params = {}) {
  return page.evaluate(
    async ({ method, params }) => {
      ...
      if (!r.ok) throw new Error(method + ": " + r.status + " " + (j.exc_type || "") + " " + (j._server_messages || ""));
      return j.message;
    },
    { method, params },
  );
...
async function page(width = 390, height = 844, auth = false) {
  const context = await browser.newContext({
    ...
    colorScheme: "light",
    ...(auth ? {recordVideo: {dir:out, size:{width:960,height:640}}} : {}),
  });
  if (auth) {
...
try {
  ...
  id = created.session; ownedSessions.push(id);
  await host.evaluate(
  ...
  await Promise.all(
    players.map((p) =>
      p
        .getByRole("button", { name: "ሌላ ጨዋታ ይቀላቀሉ", exact: true })
        .waitFor(),
    ),
  );
  ...
  const room=await api(host,"create_session",{game_key:"common-ground",configuration:{pack:"everyday",language:"am"}});
  ...
  await host.getByRole("button",{name:"ለማጋራት ዝግጁ ነን",exact:false}).waitFor();
  const roomState=await api(host,"get_host_state",{session:room.session});
  await screen.goto(`/play/room-screen/${room.game_pin}?lang=am`);
  await screen.getByRole("heading",{name:roomState.view.prompt,exact:true}).waitFor();
  await screenshot(host,"room-host-am");await screenshot(screen,"room-screen-am");
  for(let i=0;i<3;i++){
    await host.getByRole("button",{name:"ለማጋራት ዝግጁ ነን",exact:false}).click();
    await host.getByRole("button",{name:i===2?"አብረን እንጨርስ":"ቀጣይ ውይይት",exact:false}).click();
  }
  await host.getByRole("heading",{name:"ትንሽ ተጨማሪ",exact:false}).waitFor();
  await screenshot(host,"room-finish-am");
  pass("Amharic device-free host and shared-screen prompts agree; three conversations and shares finish through the UI");
} catch (e) {
  console.error("FAILED:", e.message);
  process.exitCode = 1;
  if (host) await screenshot(host, "failure-live");
} finally {
  const inventoryFile="/tmp/gp-am-sessions.json";
  const prior=JSON.parse(await fs.readFile(inventoryFile,"utf8").catch(()=>"[]"));
  await fs.writeFile(inventoryFile,JSON.stringify([...new Set([...prior,...ownedSessions])]));
  if (id)
    await fs.writeFile(
      "/tmp/gp-am-latest-network.json",
      JSON.stringify([id]),
    );
  await fs.writeFile(
  ...
  if(video && !process.exitCode) await video.saveAs(path.join(out,"working-session.webm"));
  await browser.close();
}
