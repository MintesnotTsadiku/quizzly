	...
	<div class="flex w-full flex-col items-center gap-10 text-center">
		<template v-if="view.phase === 'turn_ready' && view.performer">
			...
			<div class="flex items-center gap-10">
				<AvatarPic :id="view.performer.avatar" :nickname="view.performer.nickname" :size="150" />
				<div class="text-left">
					...
					<p class="mt-4 font-display text-3xl text-paper/60">
						{{ view.actor_team_name }} takes the stage
					</p>
				</div>
				<DrainRing :percent="timerPercent" :seconds="Math.ceil(remaining)" :size="170" color="rgb(var(--accent))" />
			</div>
		</template>

		<template v-else-if="view.phase === 'turn_open' && view.performer">
			<div class="flex items-center gap-10">
				<AvatarPic :id="view.performer.avatar" :nickname="view.performer.nickname" :size="130" />
				<div class="text-left">
					<p class="font-mono text-xs uppercase tracking-[0.3em] text-paper/40">Now performing</p>
					<h1 class="mt-1 font-display text-7xl font-extrabold leading-none text-paper">
						{{ view.performer.nickname }}
					</h1>
					<p class="mt-2 font-display text-2xl text-paper/60">{{ view.actor_team_name }}</p>
				</div>
				<div class="ml-16 flex items-center gap-10">
					<div class="text-center">
						...
						<p class="mt-1 font-mono text-xs uppercase tracking-[0.3em] text-paper/40">solved</p>
					</div>
					<DrainRing
			...
			<div class="mt-6 grid w-full max-w-6xl gap-6" :class="teamCardCols">
				<div
					...
					:class="isStageTeam(team) ? [teamStyle(team.color).border, 'scale-[1.02]'] : 'border-haze'"
				>
					<p class="truncate font-display text-2xl font-bold text-paper">{{ team.team_name }}</p>
					<p class="mt-2 font-display text-6xl font-extrabold tabular-nums text-paper/85">{{ team.score }}</p>
		...
		<template v-else-if="view.phase === 'turn_review'">
			<h2 class="font-display text-5xl font-extrabold text-paper">
				{{ solvedCount }} solved for {{ view.actor_team_name }}
			</h2>
			<div class="flex max-w-5xl flex-wrap justify-center gap-3">
				<span
					...
					:class="isSolved(word, index) ? 'bg-lagoon/25 text-ok' : 'bg-dusk text-paper/35 line-through'"
				>
					{{ word }}
				</span>
			</div>
			<p class="font-mono uppercase tracking-widest text-paper/40">{{ view.passed_count || 0 }} passed</p>
		</template>
	</div>
</template>
