<template>
	<div class="flex w-full flex-col items-center gap-6 text-center">
		...
		<template v-if="view.phase === 'turn_ready' && view.is_performer">
			...
			<h1 class="font-display text-4xl font-extrabold leading-tight text-paper">Get ready to act!</h1>
			<p class="text-paper/50">{{ view.mode }} mode</p>
		...
		<template v-else-if="['turn_ready', 'turn_open'].includes(view.phase)">
			...
			<div class="flex items-center gap-4">
				...
				<div class="text-left">
					...
					<p class="text-paper/50">{{ view.actor_team_name }} is performing</p>
				</div>
			</div>
		...
		<template v-else-if="view.phase === 'turn_review'">
			<h1 class="font-display text-4xl font-extrabold text-paper">{{ solvedCount }} solved!</h1>
