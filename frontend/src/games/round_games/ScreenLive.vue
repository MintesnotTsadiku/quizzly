<template>
	<div class="flex w-full max-w-6xl flex-col items-center gap-8 text-center">
		...
		<img
			...
			:src="view.media_url"
			:alt="$t('Round clue')"
			class="max-h-[42vh] rounded-3xl object-contain"
		/>
		...
		<p
			v-if="view.phase === 'round_reveal'"
			class="rounded-2xl bg-accent px-8 py-4 font-display text-4xl font-extrabold text-sunk"
		>
			{{ $t(view.answer || "Responses locked") }}
		</p>
		<div v-else class="flex items-center gap-10">
			...
			<p class="font-display text-5xl font-extrabold text-paper">
				{{ view.responses || 0
					...
					$t(view.phase === "vote_open" ? "choices" : "responses")
				}}</span>
			</p>
		</div>
	</div>
